...
combined_model.clk = 1:1:combined_model.num_samples;
...
model1.Q = zeros(combined_model.num_samples*(ro+1), ro+1);
model2.Q = Q2;
...
model1.R = zeros(1, combined_model.num_samples);
model2.R = R2;
for t=1:combined_model.num_samples
    if t>pulse_start && t<pulse_start + d
        model1.R(t) = sigmaV^2;
        combined_model.R(t) = model1.R(t) + R2;
    else
...
model1.P0 = eye(ro+p);
model1.x_0 = zeros((ro+p)*combined_model.num_rhythms, 1);
model2.P0 = eye(combined_model.phase_model_dimensions);
model2.x_0 = zeros(combined_model.phase_model_dimensions*combined_model.num_rhythms,1);
